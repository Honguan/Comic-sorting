...
- 分別顯示主佇列完成數與 BallonsTranslator 當前偵測／OCR／翻譯／修補百分比。停止會終止目前翻譯程序，佇列匯出會在下一張圖片檢查取消並保留既有 CBZ，清理則完成當前步驟後停止。
- 佇列與狀態會儲存，重新開始只執行「等待」項目。可上移／下移、重試選取、清除已完成；再次主動加入相同路徑與動作會將該工作重設為等待，不新增重複列。
- 雙擊佇列項目可查看完整路徑與錯誤詳情，包括翻譯程序最後 20 行輸出。失敗或停止的前置工作須先重試，才能繼續同路徑的後續工作。逗號路徑受 BallonsTranslator CLI 限制，會明確報錯。
